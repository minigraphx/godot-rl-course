...
!!! check "Done when"
    In TensorBoard, `ppo_stacked` climbs decisively while `ppo_blind` plateaus well below it — the same memory-vs-no-memory gap as §7's comparison, reproduced with the cheapest form of memory. Runs are noisy, so expect the ordering, not exact curves. If the two runs look identical, verify `MaskVelocity` is applied in both: the base env's `observation_space` should have shape `(2,)`, not `(4,)`.
...
??? success "Self-check answers"
    ...
    4. Unrolling the full episode would mean storing activations and backpropagating through potentially thousands of steps — memory blows up and gradients vanish or explode. **Truncated BPTT** backpropagates only through fixed-length chunks (capped by the `n_steps=512` rollout; minibatching splits them further), keeping updates cheap and stable while the hidden state still flows forward across chunk boundaries.
