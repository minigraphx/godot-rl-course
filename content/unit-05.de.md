...
??? success "Antworten zum Selbstcheck"
    ...
    4. Die **Seed-Varianz** im RL ist enorm — identische Hyperparameter können bei der Konvergenz um 50 % in `ep_rew_mean` auseinanderliegen, eine einzelne „verbesserte" Kurve kann also schlicht ein glücklicher Seed sein. Erst Mittelwert ± Standardabweichung über 3–5 gepaarte Seeds trennt einen echten Effekt von Zufall.
    5. Eine Verdopplung von `n_parallel` verdoppelt den Rollout-Buffer; bleibt `batch_size` fix, gleicht PPO das mit **doppelt so vielen kleinen Updates** pro Epoche aus — jeder Gradient mittelt weiterhin nur über einen dünnen Ausschnitt des nun vielfältigeren Buffers, und die längere Kette kleiner Updates lässt die Policy pro Rollout weiter driften. Wer `batch_size` mitwachsen lässt, hält die Update-Zahl stabil und lässt jeden Gradienten tatsächlich über die zusätzliche Vielfalt mitteln.
