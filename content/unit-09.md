...
??? success "Self-check answers"
    ...
    5. Open answer — example: for a sparse-reward, hard-exploration environment like **MultiLevelRobot**, demos *help*, because random exploration can't discover the route. For a simple dense-reward environment from the early units, mediocre demos could *hurt* by anchoring the policy to suboptimal behaviour that from-scratch PPO would outgrow on its own.
