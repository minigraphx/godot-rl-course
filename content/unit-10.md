...
??? success "Self-check answers"
    ...
    4. Choose **HTML5/WASM** when you want anyone to play via a URL without installing anything. It costs hosting constraints — the host must serve the `SharedArrayBuffer` (COOP/COEP) headers, which itch.io offers as an opt-in — and performance headroom, which is why the viz checkpoint tells you to test on mobile and a low-end laptop.
    5. Run the Section 9 **viz checkpoint**: play the exported build for several minutes. ONNX inference is deterministic — the same observation always produces the same action — so behavior is repeatable across runs of the build; expect it slightly smoother than training rollouts, which sampled actions stochastically. Gross misbehavior (jitter, freezing, wandering) signals an export problem, not randomness.
