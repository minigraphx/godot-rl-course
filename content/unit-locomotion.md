...
|------------------------|------------------|---------------|
| Walker (biped) | `RigidBody3D` torso + 2 legs via `HingeJoint3D` | Balance while moving forward |
| Crawler (quadruped, no trunk) | 4 independent limbs on shared base | Coordination without a central body |
| Worm | Chain of `RigidBody3D` segments, 1-DOF joints | Exploit ground friction with no legs |
...
Add a `Area3D` at each foot's bottom surface. Connect `body_entered` → a flag `foot_contact_L` / `foot_contact_R`. These go into the observation and the reward.
...
Follow the egocentric pattern from the [Robot Observations unit](unit-robotics.md#9--locomotion-observation-patterns). For the biped:
...
func get_obs() -> Dictionary:
    ...
    obs.append(vel.dot(fwd)          / MAX_SPEED)      # forward speed (key signal)
    ...
    obs.append(sin(torso.rotation.y))                   # yaw as sin/cos — avoids ±π wrap
    obs.append(cos(torso.rotation.y))
    ...
    # Per-joint state (angle + velocity) × 6 joints
    for jname in ["hip_l", "knee_l", "ankle_l", "hip_r", "knee_r", "ankle_r"]:
        var j = joints[jname]
        obs.append(j.get_param_x(Generic6DOFJoint3D.PARAM_ANGULAR_LOWER_LIMIT) / PI)
        obs.append(j.get_param_x(Generic6DOFJoint3D.PARAM_ANGULAR_MOTOR_TARGET_VELOCITY) / MAX_JOINT_VEL)

    # Foot contact (binary)
    obs.append(1.0 if foot_l.foot_contact else 0.0)
    obs.append(1.0 if foot_r.foot_contact else 0.0)

    # Target speed command (lets you train a single policy at multiple speeds)
...
Nine continuous outputs — one per controlled DOF. The joint motors accept a **target velocity**; the motor limit caps the force applied.

```gdscript
func get_action_space() -> Dictionary:
    # 9 outputs: hip_l (x,y,z), knee_l (x), ankle_l (x), hip_r (x,y,z), knee_r (x), ankle_r (x)
    return {"joints": {"size": 9, "action_type": "continuous"}}

func set_action(action) -> void:
    var a = action["joints"]
    var MAX_VEL = 6.0   # rad/s — tune to your joint limits
    ...
    # ankle_r shares the knee_r slot for a 9-output policy — or add a 10th output
```
...
func _physics_process(delta):
    ...
    _compute_reward(delta)
    _check_termination()

func _compute_reward(delta):
    var fwd     = -torso.global_transform.basis.z
    var vel     = torso.linear_velocity
    var fwd_vel = vel.dot(fwd)           # positive = moving forward

    # 1. Forward velocity reward — the primary drive
    ...
    var r_upright = (up_dot - 0.5) * 0.1   # zero at 60° tilt, positive above

    # 4. Alive bonus — small per-step reward so the agent prefers to stay alive
    var r_alive = 0.002

    _ai.reward += r_vel + r_energy + r_upright + r_alive

func _check_termination():
    # Fall detection: torso too low
    if torso.global_position.y < 0.5:
        _alive = false
        _ai.reward -= 1.0    # fall penalty
        _ai.done = true
```
...
| Energy penalty | Agent learns to vibrate joints at max speed (wastes energy, looks robotic) | Smooth, efficient gait |
| Upright bonus | Agent crawls on its face or hops sideways | Stays balanced |
| Alive bonus | Agent learns to fall immediately to end bad episodes | Prefers longer episodes where it can accumulate rewards |

!!! warning "Coefficient ordering matters"
    Keep `r_vel` in the range [0, 1]. All other terms should be ≤ 10% of the forward velocity signal. If the energy penalty dominates, the agent stands still (zero energy = zero penalty). If upright dominates, the agent balances without walking.
...
`n_steps=2048` gives PPO long rollouts — locomotion episodes last hundreds of steps and the advantage estimate needs time to propagate backward through a gait cycle.
...
### The statue
Agent stands perfectly still. `ep_rew_mean` > 0 but `ep_len_mean` is maxing out at the episode timeout.
...
### The hopper
Agent learns a single leg bounce — works, but looks nothing like the ML-Agents demos.
...
**Fix:** Add a **foot alternation bonus**: reward when the left and right contact signals alternate (not both on, not both off). `r_contact = abs(foot_l - foot_r) * 0.05`.
...
**Fix:** Spawn the body lower (torso at 0.8 m instead of 1.2 m), or add a 0.5-second freeze at episode start so physics settle before the policy begins.
...
- 4 legs × 2 joints each = 8-DOF action space
- Add all 4 foot contact signals
- Increase `n_parallel` — quadruped training typically needs 32+ parallel envs for sample efficiency
...
- Forward velocity is measured from the head segment
- Worm discovers a sinusoidal wave pattern around 3–5M steps — clearly visible in the viz checkpoint

### Variable-speed command

To train a **single policy** that can walk at multiple speeds (like the ML-Agents Walker demo), randomize `target_speed` per episode:
...
**3M:** Upright but awkward. Shuffles forward. Falls occasionally. This is the hardest phase to watch — it looks like a drunk person. It's working.
