...
??? success "Self-check answers"
    ...
    2. Reach for a **shared policy** when agents are cooperative and interchangeable — identical `get_obs()` and `get_action_space()` — it cuts sample requirements and softens non-stationarity — "other agents" and "self" are the same network, so everyone's behavior shifts consistently, though the shared policy still changes with every update. Use **independent policies** for competitive or differently-skilled agents. Different observation shapes rule out sharing entirely: you must instantiate separate `StableBaselinesGodotEnv` wrappers, each with its own model.
