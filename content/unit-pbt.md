...
!!! check "Done when"
    `tuner.fit()` runs all eight trials to the 500k-step stop — watch them appear and report `mean_reward` in the Ray Tune dashboard at `http://localhost:8265` — and prints a best config and reward at the end. The real success signal is the trajectory plot from Section 7: the hyperparameter lines show vertical steps at perturbation events. Survivors often drift toward lower `learning_rate` too, but with only ~5 perturbation cycles in a 500k-step budget that drift is not guaranteed — don't read its absence as failure (Section 8 notes the static-vs-dynamic gap is small at this scale). If every trajectory is a flat line with no jumps, the exploit step never fired — check `perturbation_interval` against your total step budget and the `metric`/`mode` settings, as described in Section 7.
