...
!!! note "First pass? Skim or skip this section."
    The hands-on path runs through Sections 1–8 (the model-free vs model-based trade-off, what a world model learns, the RSSM architecture, DreamerV3, Godot integration, when world models win and lose, and latent space visualization). This section is historical context — only the "Dyna from scratch" stretch goal builds on it, so come back here before attempting that one.
